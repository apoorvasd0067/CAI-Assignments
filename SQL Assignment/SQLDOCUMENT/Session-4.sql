...
ON T.EmployeeNumber = S.EmployeeNumber AND T.TransactionDate = S.TransactionDate
WHEN MATCHED THEN
    UPDATE SET Amount = T.Amount + S.Amount
WHEN NOT MATCHED BY TARGET THEN
    INSERT ([Amount], [TransactionDate], [EmployeeNumber])
	VALUES (S.Amount, S.TransactionDate, S.EmployeeNumber);
ROLLBACK TRAN

--8.  Let’s expand our MERGE statement
SELECT TransactionDate, EmployeeNumber, COUNT(*) AS NumberOfRows
FROM tblTransactionNew 
GROUP BY TransactionDate, EmployeeNumber
HAVING COUNT(*)>1
...
GO
MERGE INTO tblTransaction as T
USING (SELECT TransactionDate, EmployeeNumber, MIN(Amount) as Amount
      FROM tblTransactionNew
	  GROUP BY TransactionDate, EmployeeNumber) as S
...
USING (select EmployeeNumber, TransactionDate, sum(Amount) as Amount
from tblTransactionNew
group by EmployeeNumber, TransactionDate) as S
ON T.EmployeeNumber = S.EmployeeNumber AND T.TransactionDate = S.TransactionDate
...
WHEN MATCHED THEN
	DELETE
WHEN NOT MATCHED BY TARGET THEN
    INSERT ([Amount], [TransactionDate], [EmployeeNumber], Comments)
	VALUES (S.Amount, S.TransactionDate, S.EmployeeNumber, 'Inserted Row')
